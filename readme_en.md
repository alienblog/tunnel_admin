...
| Module | Capabilities |
|---|---|
| Web Terminal | Multi-tab/split-pane (VSCode editor group model), drag-and-drop docking (**zero reconnect on layout changes**), **auto-reconnect** on disconnect (exponential backoff, history preserved), ws-reconnect resume, tmux session persistence (survives refresh/disconnect), `clear` keeps history (scrolls to top), Ctrl+F search, command completion, multi-line paste confirmation, theme presets, session recording & replay, connection status badge |
| Host Management | Groups/tags/notes, credentials encrypted at rest with AES-256-GCM, per-host trust switch, **double-click** to open terminal in sidebar, modal edit dialog (ESC/button to close) |
...
| **Total (1 session)** | **642,049** | **1,249,697** | **629,155,968** | **$2.20** |
<!-- /usage -->
