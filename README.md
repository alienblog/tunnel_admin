...
| 模块 | 能力 |
|---|---|
| Web 终端 | 多标签/分屏（VSCode 编辑器组模型，**第一层 tab 同样可拖拽分屏/合并**）、**工具页可拖到右缘停靠为固定右栏**（可折叠/移回/**拖动调宽**）、拖拽停靠（**布局重组零重连**）、断线**自动重连**（指数退避，历史不丢）、ws 断线补发恢复、`clear` 不清屏（滚动到顶部）、Ctrl+F 搜索、命令补全、多行粘贴确认、主题预设、会话录制回放、连接状态徽标 |
...
| 插件系统 | VSCode 模型：独立项目插件的插件清单（`plugin.json`）+ `server` 后端扩展 + `web` 前端页面，**本地目录加载**（开发模式）或 **`.taplugin` 包安装**；插件页面 iframe 承载（`ta.api` 宿主桥）、私有 KV 存储（AES 加密）、加密配置、定时任务、自定义路由、**一键 SSH 连接**（`ctx.ssh.requestConnect` + `ta.ssh.connect`，明文凭据不出服务端，支持跳板机）；`node scripts/package-plugin.cjs <目录>` 打包 |
...
| 会话 | 输入 | 输出 | 缓存读 | 成本 |
|---|---|---|---|---|
| 1 | 4,780,042 | 2,163,528 | 985,048,448 | $4.03 |
| 2 | 23,393 | 2,512 | 87,680 | $0.00 |
| 3 | 1,239,327 | 413,215 | 175,162,880 | $0.48 |
| **累计（3 个会话）** | **6,042,762** | **2,579,255** | **1,160,299,008** | **$4.52** |
<!-- /usage -->
