...
| 插件系统 | VSCode 模型：独立项目插件的插件清单（`plugin.json`）+ `server` 后端扩展 + `web` 前端页面，**本地目录加载**（开发模式）或 **`.taplugin` 包安装**；插件页面 iframe 承载（`ta.api` 宿主桥）、私有 KV 存储（AES 加密）、加密配置、定时任务、自定义路由；`node scripts/package-plugin.cjs <目录>` 打包 |
...
| 会话 | 输入 | 输出 | 缓存读 | 成本 |
|---|---|---|---|---|
| 1 | 4,544,106 | 1,959,595 | 911,004,544 | $3.74 |
| 2 | 23,393 | 2,512 | 87,680 | $0.00 |
| **累计（2 个会话）** | **4,567,499** | **1,962,107** | **911,092,224** | **$3.74** |
<!-- /usage -->
